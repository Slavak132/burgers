...
task("default", series("clean", "styles", "server"));
...
// task('build', series('clean', parallel('copy:html', 'sass:style', 'scripts:js', 'svg:icons', 'copy:png', 'copy:jpeg', 'copy:video')));//Так же установлен плагин npm install jquery npm install cross-env --save-dev(пакет определяющий окружение)
// const {src, dest, task, series, watch, parallel} = require('gulp'),
//       rm = require( 'gulp-rm' ), //Очистка дирректории (dist)
//       sass = require('gulp-sass'), //Sass компиляция
//       concat = require('gulp-concat'), //Склейка файлов
